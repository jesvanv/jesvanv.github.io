...
var fileBottomNavCloseImage = "images/closelabel_crop.png";		// Close image
	...
	start: function(elementLink) {	
		...
		if((elementLink.getAttribute('rel') == 'lightbox')){
			...
			if(elementLink.getAttribute('href').endsWith('swf')) { // SWF
				elementArray.push(new Array(elementLink.getAttribute('href'), elementLink.getAttribute('lightbox_title'), elementLink.getAttribute('width'), elementLink.getAttribute('height')));
			} else { // Image
				elementArray.push(new Array(elementLink.getAttribute('href'), elementLink.getAttribute('lightbox_title')));
			}				
		} else {
		// Image or SWF is not part of a set...
				
			// Loop through anchors, find other images/lightbox/SWF's in the set, and add them to elementArray
			for (var i=0; i<anchors.length; i++){
				var anchor = anchors[i];
				if (anchor.getAttribute('href') && (anchor.getAttribute('rel') == elementLink.getAttribute('rel'))){
					if(anchor.getAttribute('href').endsWith('swf')) { // SWF
						elementArray.push(new Array(anchor.getAttribute('href'), anchor.getAttribute('lightbox_title'), anchor.getAttribute('width'), anchor.getAttribute('height')));
					} else {
						elementArray.push(new Array(anchor.getAttribute('href'), anchor.getAttribute('lightbox_title')));
					}
				}
			}
